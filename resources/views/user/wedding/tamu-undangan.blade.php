    ...
    <div class="relative w-full flex items-start justify-center gap-4">
                            ...
                            <td>
                                <div class="flex items-center justify-center gap-1 text-xs">
                                    ...
                                    @if ($today->lt($eventDate))
                                        <form action="{{ route('user.tamu.kirim', $tamu->id) }}" method="post" class="validasiKirimUndanganForm">
                                            @csrf
                                    ...
                                    {{-- HAPUS --}}
                                    @if ($today->lt($eventDate))
                                        <form action="{{ route('user.tamu.hapus', $tamu->id) }}" method="post" class="hapusUndanganForm">
                                            @csrf
        ...
        @if ($today->lt($eventDate))
            ...
            <div class="sticky top-4 w-[300px] bg-white rounded-xl p-4 border shadow-sm text-sm">
                <form action="{{ route('user.tamu.tambah', $wedding->id) }}" method="post">
                    @csrf
                    {{-- NAMA --}}
