        ...
        <div class="w-3/4 mx-auto mb-4 p-4 rounded border-2 border-slate-100">
            ...
            <div class="w-full pt-4 flex items-center justify-between border-t-2 border-slate-100">
                ...
                <div class="flex-1 w-full flex items-center justify-end gap-4">
                    <button class="openFtgModalBtn w-fit px-4 py-2 rounded outline-none bg-pink text-white hover:bg-pink-hover focus:bg-pink-hover active:bg-pink-active transition-colors"
        ...
        <div class="fixed top-full left-0 w-full h-screen flex items-center justify-center bg-slate-500/50 z-10 transition-all duration-500"
            ...
            <div class="max-w-[1200px] bg-white rounded-md">
                ...
                <div class="w-full p-4 flex items-center justify-between">
                    ...
                    {{-- TOMBOL CLOSE MODAL --}}
                    <div>
                        <button class="closeFtgModalBtn w-fit px-4 aspect-square rounded outline-none bg-pink text-white hover:bg-pink-hover focus:bg-pink-hover active:bg-pink-active transition-colors"
                        ...
                        <div class="w-full">
                            ...
                            <div class="w-full mb-4 flex items-center justify-start gap-4">
                                ...
                                <a class="block text-2xl font-semibold outline-none underline"
                                    href="{{ route('wedding-couple.search.wo.ke_detail', $bp->plan->w_photographer->id) }}" tabindex="-1">
                                    {{ $bp->plan->w_photographer->nama }}
                                </a>
                            </div>
                                ...
                                <div class="w-full mb-4">
                                    ...
                                    <div class="w-full flex items-stretch justify-center rounded-md border-2 border-slate-300">
                                        ...
                                        <button class="copyToClipBtn w-10 aspect-square p-2 rounded-e outline-none bg-slate-300 hover:bg-slate-200 focus:bg-slate-200 transition-colors"
                                            data-value="{{ $bp->plan->w_photographer->no_rekening }}">
                                ...
                                @else
                                    <form action="{{ route('wedding-couple.pernikahan.upload_bukti_bayar_wp', $bp->id) }}" method="post" enctype="multipart/form-data" id="upBuktiBayarFtgForm-{{ $bp->id }}">
                                        ...
                                        <div class="w-full flex items-center justify-center gap-2">
                                            <button class="upBuktiBayarFtgBtn flex-1 w-full px-4 py-2 rounded outline-pink outline-offset-2 bg-white text-pink hover:bg-pink hover:text-white focus:bg-pink focus:text-white active:bg-pink-active transition-colors"
                                                type="button" data-id="{{ $bp->id }}">
                                                Pilih Gambar
                                            </button>
                                            <button class="submitBuktiBayarFtgBtn flex-1 w-full px-4 py-2 rounded outline-pink outline-offset-2 bg-pink text-white hover:bg-pink-hover focus:bg-pink-hover active:bg-pink-active disabled:outline-slate-200 disabled:bg-slate-200 disabled:text-slate-500 disabled:cursor-not-allowed transition-colors"
                                                type="button" id="submitBuktiBayarFtgBtn-{{ $bp->id }}" data-id="{{ $bp->id }}" disabled>
                                                Unggah Gambar
                                            </button>
                                        </div>
                                    </form>
                    ...
                    <div class="w-fit text-sm">
                        ...
                        @if ($bp->status != 'selesai')
                            <form action="{{ route('wedding-couple.pernikahan.hapus_wp', $bp->id) }}" method="post" id="hapusWPForm-{{ $bp->id }}">
                                @csrf
                                <button class="hapusWPBtn w-fit px-4 py-2 rounded outline-none bg-pink text-white hover:bg-pink-hover focus:bg-pink-hover active:bg-pink-active transition-colors"
