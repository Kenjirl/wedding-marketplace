...
@section('body')
    <div class="w-full max-w-[1200px] max-h-screen mx-auto overflow-y-auto bg-white">
        <div class="min-h-screen flex items-center justify-center"
            ...
            id="header">
            @include('user.wedding-couple.undangan.template.header.'.$wedding->invitation->t_header)
        </div>
        ...
        @if ($wedding->invitation->t_quote != 0)
            <div class="w-full flex items-center justify-center"
                ...
                id="quote">
                @include('user.wedding-couple.undangan.template.quote.'.$wedding->invitation->t_quote)
            </div>
        @endif
        ...
        <div class="flex items-center justify-center"
            ...
            id="profile">
            @include('user.wedding-couple.undangan.template.profile.'.$wedding->invitation->t_profile)
        </div>

        <div class="min-h-[50vh] flex items-center justify-center"
            style="background-color: {{ $wedding->invitation->c_event['div'] }}; color: {{ $wedding->invitation->c_event['text'] }};"
            id="event">
            @include('user.wedding-couple.undangan.template.event.'.$wedding->invitation->t_event)
        </div>
        ...
        @if ($wedding->invitation->t_gallery != 0)
            <div class="min-h-[50vh] flex items-center justify-center"
                ...
                id="gallery">
                @include('user.wedding-couple.undangan.template.gallery.'.$wedding->invitation->t_gallery)
            </div>
        @endif
        ...
        <div class="min-h-screen flex items-center justify-center"
            ...
            id="wish">
            @include('user.wedding-couple.undangan.template.wish.'.$wedding->invitation->t_wish)
        </div>

        <div class="min-h-screen flex items-center justify-center"
            style="background-color: {{ $wedding->invitation->c_footer['div'] }}; color: {{ $wedding->invitation->c_footer['text'] }};"
            id="footer">
            @include('user.wedding-couple.undangan.template.footer.'.$wedding->invitation->t_footer)
        </div>
    </div>
@endsection
