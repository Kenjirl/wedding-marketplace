...
@section('content')
    ...
    <form action="{{ route('admin.portofolio.validasi', ['vendor' => $vendor, 'id' => $portofolio->id]) }}" method="post" id="validasiForm">
        @csrf
        ...
        <div class="w-full mt-4 flex items-center justify-end gap-4">
            <a class="w-fit px-4 py-2 font-semibold outline-none text-pink bg-white hover:bg-pink hover:text-white focus:bg-pink focus:text-white active:bg-pink-active transition-colors rounded"
                href="{{ route('admin.portofolio.index', ['vendor' => $vendor, 'tab' => $portofolio->status]) }}">
