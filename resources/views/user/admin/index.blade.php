            ...
            <div class="w-1/2 rounded-lg shadow-md">
                ...
                <div class="w-full h-[300px] overflow-y-auto">
                    @forelse ($portofolios as $portofolio)
                        @php
                            $vendorIcon = $vendors[$portofolio->w_vendor->jenis]['icon'] ?? 'fa-regular fa-question-circle';
                            $statusIcon = $statuses[$portofolio->status]['icon'] ?? 'fa-regular fa-question-circle';
                        @endphp

                        <div class="w-full p-4 flex items-center justify-between gap-4">
                            <div class="w-full line-clamp-1">
                                {{ $portofolio->judul }}
                            </div>
                            <div class="w-[50px] aspect-square flex items-center justify-center rounded shadow-md bg-pink text-white"
                                data-tippy-content="{{ $vendors[$portofolio->w_vendor->jenis]['name'] }}">
                                <i class="{{ $vendorIcon }}"></i>
                            </div>
                            <div class="w-[50px] aspect-square flex items-center justify-center rounded shadow-md bg-{{ $statuses[$portofolio->status]['color'] }}-400 text-white"
