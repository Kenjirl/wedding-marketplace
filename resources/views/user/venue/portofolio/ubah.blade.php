...
@section('content')
    ...
    <form action="{{ route('venue.portofolio.ubah', $portofolio->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        {{-- INPUTS --}}
    ...
    <form class="hidden" action="{{ route('venue.portofolio.hapus', $portofolio->id) }}" method="post">
        @csrf
    ...
    @foreach ($portofolio->foto as $index => $foto)
        <form id="deleteImageForm-{{ $index }}" action="{{ route('venue.portofolio.hapus-foto', ['id' => $portofolio->id, 'index' => $index]) }}" method="post">
            @csrf
        </form>
    @endforeach
@endsection
