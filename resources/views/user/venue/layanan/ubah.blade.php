...
@section('content')
    <form action="{{ route('venue.layanan.ubah', $plan->id) }}" method="post" enctype="multipart/form-data" autocomplete="off">
        @csrf
        <div class="w-full">
            {{-- INPUT --}}
                ...
                <div class="w-full">
                    ...
                    @php
                        $satuanVenue = [
                            ...
                            'Paket' => ['paket meeting', 'paket pernikahan', 'paket ulang tahun'],
                    ...
                    <div class="w-full mb-4">
                        <div class="w-full">
                            ...
                            <select class="w-full p-2 flex-1 border-x-2 border-b-2 text-sm @error('gender') border-red-500 @enderror rounded-b focus:border-pink outline-none"
                                ...
                                @foreach ($satuanVenue as $category => $options)
    ...
    {{-- FORM HAPUS LAYANAN --}}
    <form class="hidden" action="{{ route('venue.layanan.hapus', $plan->id) }}" method="post">
        @csrf
    ...
    @foreach ($plan->foto as $index => $foto)
        <form id="deleteImageForm-{{ $index }}" action="{{ route('venue.layanan.hapus-foto', ['id' => $plan->id, 'index' => $index]) }}" method="post">
            @csrf
        </form>
    @endforeach
@endsection
