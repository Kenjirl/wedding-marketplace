...
@section('content')
    ...
    <form action="{{ route('catering.portofolio.ubah', $portofolio->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        {{-- INPUTS --}}
    ...
    <form class="hidden" action="{{ route('catering.portofolio.hapus', $portofolio->id) }}" method="post">
        @csrf
    ...
    @foreach ($portofolio->foto as $index => $foto)
        <form id="deleteImageForm-{{ $index }}" action="{{ route('catering.portofolio.hapus-foto', ['id' => $portofolio->id, 'index' => $index]) }}" method="post">
            @csrf
        </form>
    @endforeach
@endsection
