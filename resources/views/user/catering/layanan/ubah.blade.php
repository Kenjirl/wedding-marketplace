...
@section('content')
    <form action="{{ route('catering.layanan.ubah', $plan->id) }}" method="post" enctype="multipart/form-data" autocomplete="off">
        @csrf
        <div class="w-full">
            {{-- INPUT --}}
    ...
    {{-- FORM HAPUS LAYANAN --}}
    <form class="hidden" action="{{ route('catering.layanan.hapus', $plan->id) }}" method="post">
        @csrf
    ...
    @foreach ($plan->foto as $index => $foto)
        <form id="deleteImageForm-{{ $index }}" action="{{ route('catering.layanan.hapus-foto', ['id' => $plan->id, 'index' => $index]) }}" method="post">
            @csrf
        </form>
    @endforeach
@endsection
