...
class VJadwalController extends Controller
{
    public function index() {
        ...
        foreach ($bookings as $booking) {
            $events[] = [
                ...
                'allDay' => true,
                'url'    => route('venue.jadwal.ke_detail', $booking->id),
                'start' => $booking->untuk_tanggal,
            ];
        }

        return view('user.venue.jadwal.index', compact('events'));
    }
    ...
    public function ke_detail($id) {
        ...
        if (!$booking) {
            return redirect()->route('venue.jadwal.index')->with('gagal', 'ID Invalid');
        }
        ...
        return view('user.venue.jadwal.detail', compact(
            'booking',
            'plan',
            'wedding',
            'events',
    ...
    public function batal(Request $req, $id) {
        ...
        if ($data) {
            return redirect()->route('venue.jadwal.index')->with('sukses', 'Membatalkan Pesanan');
        }
        return redirect()->route('venue.jadwal.ke_detail', $id)->with('gagal', 'Terjadi kesalahan');
    }
}
