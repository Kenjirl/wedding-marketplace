...
class VPesananController extends Controller
{
    public function index() {
        ...
        return view('user.venue.pesanan.index', compact('bookings'));
    }
    ...
    public function ke_detail($id) {
        ...
        if (!$booking) {
            return redirect()->route('venue.pesanan.index')->with('gagal', 'ID Invalid');
        }
        ...
        return view('user.venue.pesanan.detail', compact(
            'booking',
            'plan',
            'wedding',
            'events',
    ...
    public function respon(Request $req, $id) {
        ...
        if ($data && $req->status == 'diterima') {
            return redirect()->route('venue.pesanan.index')->with('sukses', 'Menerima Pesanan');
        } else if ($data && $req->status == 'ditolak') {
            return redirect()->route('venue.pesanan.index')->with('sukses', 'Menolak Pesanan');
        }
        return redirect()->route('venue.pesanan.ke_detail', $id)->with('gagal', 'Terjadi kesalahan');
    }
}
