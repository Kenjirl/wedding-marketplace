...
class CtBookingController extends Controller
{
    public function index(Request $req) {
        ...
        foreach ($caterings as $catering) {
            ...
            $catering->harga_terendah = $plans->min('harga');
            $catering->harga_tertinggi = $plans->max('harga');
            $catering->rate = 0;
            $catering->bookedCount = 0;
            ...
            if ($bookings->isNotEmpty()) {
                $catering->rate = WVRating::whereIn('w_v_booking_id', $bookings->pluck('id'))->avg('rating');
                $catering->bookedCount = BookingFormatter::format($bookings->count());
            }

            $catering->harga_terendah = NumberFormatter::format($catering->harga_terendah);
            $catering->harga_tertinggi = NumberFormatter::format($catering->harga_tertinggi);
        }
        ...
        $filteredCaterings = $caterings->filter(function($catering) use ($search_harga, $search_basis_operasi, $search_kota_operasi) {
            if ($search_harga !== null && $catering->harga_terendah > $search_harga) {
                return false;
            }

            if ($search_basis_operasi !== null && strpos(strtolower($catering->basis_operasi), strtolower($search_basis_operasi)) === false) {
                return false;
            }

            if ($search_kota_operasi !== null && strpos(strtolower($catering->kota_operasi), strtolower($search_kota_operasi)) === false) {
                return false;
            }
        ...
        return view('user.wedding-couple.booking.ct.index', [
            'filteredCaterings' => $filteredCaterings,
    ...
    public function ke_detail(Request $req, $id) {
        ...
        // DATA PLAN
        $plans = WVPlan::where('w_vendor_id', $catering->id)
                    ->orderBy('harga', 'asc')
        ...
        $catering->rate = $totalReviewCountAllPlans ? $totalRatingAllPlans / $totalReviewCountAllPlans : null;

        if ($tab == 'portofolio') {
            // DATA PORTOFOLIO
            $portofolios = WVPortofolio::where('w_vendor_id', $catering->id)
            ...
            $bookedWeddingIds = WVBooking::whereIn('w_v_plan_id', $ctPlanIds)
    ...
    public function pesan(Request $req) {
        ...
        if ($data) {
            return redirect()->route('wedding-couple.pernikahan.ke_detail', $req->wedding_id)->with('sukses', 'Memesan Wedding catering');
        }
        return redirect()->route('wedding-couple.pernikahan.ke_detail', $req->wedding_id)->with('gagal', 'Memesan Wedding catering');
    }
}
