...
class WCInvitationController extends Controller
{
    public function ke_tambah(Request $req) {
        ...
        foreach ($folders as $folder) {
            $path = resource_path("views/user/wedding-couple/undangan/template/$folder");
            if (File::exists($path)) {
        ...
        return view('user.wedding-couple.undangan.tambah', compact('wedding', 'f_counts'));
    }
    ...
    public function tambah(Request $req) {
        ...
        if (!$wedding) {
            return redirect()->route('wedding-couple.pernikahan.index')->with('gagal', 'ID tidak valid');
        }
        ...
        if ($invitation) {
            return redirect()->route('wedding-couple.pernikahan.ke_detail', $wedding->id)->with('sukses', 'Membuat Undangan Digital');
        }

        return redirect()->route('wedding-couple.pernikahan.ke_detail', $wedding->id)->with('gagal', 'Membuat Undangan Digital');
    }
    ...
    public function ubah(Request $req, $id) {
        $req->validate([
            ...
            'foto_galeri'   => 'required|array|min:6|max:6',
            ...
            'quote.required'       => 'Kutipan harus diisi.',
            'quote.max'            => 'Kutipan maksimal 255 karakter.',
            'author.required'      => 'Asal kutipan harus diisi.',
            'author.max'           => 'Asal kutipan maksimal 50 karakter.',
            'foto_galeri.required' => 'Harus menyertakan 6 foto.',
    ...
    public function undangan($pengantin, $link) {
        ...
        return view('user.wedding-couple.undangan.index', compact(
            'wedding',
            'tamu',
        ));
    }
