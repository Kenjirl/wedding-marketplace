...
class CtProfilController extends Controller
{
    public function index() {
        return view('user.catering.profil.index');
    }
    ...
    public function ke_ubah() {
        ...
        return view('user.catering.profil.ubah',
            compact(
                'provinsi',
                'kota',
                'kecamatan',
    ...
    public function ubah(ProfilRequest $req) {
        ...
        if (auth()->user()->w_vendor) {
            ...
            # Make New
            $catering = new WVendor();
            $catering->user_id       = auth()->user()->id;
            $catering->nama          = $req->nama;
            $catering->no_telp       = $req->no_telp;
            $catering->basis_operasi = $req->basis_operasi;
            $catering->kota_operasi  = $kota_operasi;
            $catering->alamat        = $alamat;
            $catering->jenis         = auth()->user()->role;
            $catering->rekening      = $arrRekening;
            $data = $catering->save();
        }

        if ($data) {
            return redirect()->route('catering.profil.index')->with('sukses', 'Mengubah Data Diri');
        }

        return redirect()->route('catering.profil.index')->with('gagal', 'Mengubah Data Diri');
    }

    public function ke_ubah_password() {
        return view('user.catering.profil.ubah-password');
    }
    ...
    public function ubah_password(UbahPasswordRequest $req) {
        ...
        if ($data) {
            return redirect()->route('catering.profil.index')->with('sukses', 'Mengubah Password');
        }
        return redirect()->route('catering.profil.index')->with('gagal', 'Mengubah Password');
    }

    public function ke_ubah_foto() {
        return view('user.catering.profil.ubah-foto');
    }
    ...
    public function ubah_foto(Request $req) {
        ...
        if ($req->hasFile('foto_profil')) {
            ...
            if ($data) {
                return redirect()->route('catering.profil.index')->with('sukses', 'Mengubah Foto Profil');
            }
        }
        return redirect()->route('catering.profil.index')->with('gagal', 'Mengubah Foto Profil');
    }
}
